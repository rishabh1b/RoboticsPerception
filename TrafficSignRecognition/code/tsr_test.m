...
max_error_score = 0.05; % for acceptable error margin in prediction
                        % If greater than this value, we have a false
                        % positive
...
for i = 34119:34119
    ...
    M_roi(sAll) = 1;
    figure(1)
    subplot(1,2,1), imshow(M_roi),title('MSER on only subset of image')
    M(1:500,:) = M(1:500,:) + M_roi;
    subplot(1,2,2), imshow(M), title('MSER scaled to entire image')
    ...
    figure(3)
    subplot(2,2,1), imshow(im_s_bw), title('Output using only thresholding')
    subplot(2,2,2), imshow(M), title('Output using only MSER')
    im_final = M & im_s_bw & im_v_bw;
    subplot(2,2,3), imshow(im_final), title('bitwise and on MSER and thresholding')
    %imtool(im_hsv(:,:,1));
    %imtool(im_hsv(:,:,2));
    %imtool(im_hsv(:,:,3));
...
%     imshow(im_filtered)
    ...
    subplot(2,2,4), imshow(im_erode), title('Operations after suitable morphological cleaning')
