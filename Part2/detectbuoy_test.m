...
%outputsegbasefilename = '../..Output/Part2/Frames/output_';
...
for frameid = 1 : 200
    ...
    if ~isempty(pixels_needed) % Well, do it if there is any red buoy
        [centre_window, radius_window] = getCentreAndRadius(pixels_needed);
    ...
    %Y_gmm_norm_2d = reshape(Y_gmm_norm,[m,n]);
    ...
    if isempty(pixels_needed)
        ...
        if (hypot((centre_g(2) - centre_y(2)), (centre_g(1) - centre_y(1))) < radius_y + 10) || ...
                (abs(centre_g(1) - centre_y(1)) < 10) || (abs(centre_g(1) - centre_window(1)) < 30)
            no_green_buoy = true;
        else
            th = 0:pi/50:2*pi;
            xunit_g = radius_g * cos(th) + centre_g(1);
            yunit_g = radius_g * sin(th) + centre_g(2);
        end
    end
